...
Subj=$1 #AD_CT01
dirSurf=$2 #/vols/Data/Adapticity/data/AD_CT01/gifti
dirFreesurfer=$3 #/vols/Data/A

  mkdir -p $dirSurf #make new folder, if not exists
  
  # convert orig and 001; register orig.mgz (input image) to rawavg.mgz (file generated after averaging if more than one run).
  ...
  orig=$dirFreesurfer/mri/orig/001.mgz #001.mgz is the original input to recon-all 
  mri_convert $orig $dirSurf/orig.nii.gz
  conformed=$dirFreesurfer/mri/orig.mgz #conformed across runs if input more than one run; the file registered to rawavg.mgz
  ...
  for hemi in lh rh;do
    ...
    echo "addpath ~saad/matlab/surfops" > $dirSurf/matscript.m
